...
const PageNavigation: React.FC<PaginatorProps> = ({ handlePageChange }) => {
        ...
        if (pageNum > halfMaxPages + 2) {
          newRows.push(
              <Button
                  ...
                  className="ellipsis"
                  clickHandler={() =>
                      handlePageChange(Math.max(pageNum - Math.ceil(maxPagesToShow / 2), 1))
                  }
                  aria-label="Left Ellipsis"
              />
          );
        }
        ...
        if (pageNum < totalPages - halfMaxPages - 1) {
          newRows.push(
              <Button
                  ...
                  className="ellipsis"
                  clickHandler={() =>
                      handlePageChange(Math.min(pageNum + Math.ceil(maxPagesToShow / 2), totalPages))
                  }
                  aria-label="Right Ellipsis"
              />
          );
        }
